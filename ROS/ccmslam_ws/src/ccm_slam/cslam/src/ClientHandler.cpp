...
ClientHandler::ClientHandler(ros::NodeHandle Nh, ros::NodeHandle NhPrivate,
                             ...
                             viewptr pViewer)
    : mpVoc(pVoc), mpKFDB(pDB), mpMap(pMap), mNh(Nh), mNhPrivate(NhPrivate),
      mClientId(ClientId), mpUID(pUID), mSysState(SysState),
      mstrCamFile(strCamFile), mpViewer(pViewer), mbReset(false) {
...
void ClientHandler::InitializeServer() {
  ...
  *ss << "TransOut"
  ...
  usleep(10000);
  mPubTrans =
      mNh.advertise<geometry_msgs::TransformStamped>(PubTransTopicName, 1);
  //+++++ Initialize the Local Mapping thread +++++
  mpMapping.reset(new LocalMapping(mpCC, mpMap, mpKFDB, mpViewer));
  ...
  ptrTransformStamped.reset(
      new thread(&ClientHandler::PublishTransThread, this));
  usleep(10000);
