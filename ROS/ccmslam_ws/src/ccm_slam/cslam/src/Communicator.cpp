...
Communicator::Communicator(ccptr pCC, vocptr pVoc, mapptr pMap, dbptr pKFDB)
    : mpCC(pCC), mNh(pCC->mNh), mNhPrivate(pCC->mNhPrivate), mpVoc(pVoc),
      mpMap(pMap), mpDatabase(pKFDB), mClientId(pCC->mClientId),
      mbResetRequested(false), mNearestKfId(defpair), mpNearestKF(nullptr),
      mdPeriodicTime((pCC->mSysState == eSystemState::CLIENT)
                         ? params::comm::client::mfPubPeriodicTime
                         : params::comm::server::mfPubPeriodicTime),
      mdLastTimePub(0.0), mnMaxKfIdSent(0),
      mKfItBound((pCC->mSysState == eSystemState::CLIENT)
                     ? params::comm::client::miKfItBound
                     : params::comm::server::miKfItBound),
      mMpItBound((pCC->mSysState == eSystemState::CLIENT)
                     ? params::comm::client::miMpItBound
                     : params::comm::server::miMpItBound),
      mKfItBoundPub(params::comm::client::miKfPubMax),
      mMpItBoundPub(params::comm::client::miMpPubMax), mnEmptyMsgs(0) {
  ...
  if (mpCC->mSysState == eSystemState::CLIENT) {
    ...
    mSubMap = mNh.subscribe<ccmslam_msgs::Map>(
        MapInTopicName, params::comm::client::miSubMapBufferSize,
        boost::bind(&Communicator::MapCbClient, this, _1));

    // Publisher
    ss = new stringstream;
    *ss << "MapOut" << SysType << mClientId;
    PubMapTopicName = ss->str();
    mPubMap = mNh.advertise<ccmslam_msgs::Map>(
        PubMapTopicName, params::comm::client::miPubMapBufferSize);
    ...
    *ss << "MapInTopicName" << mClientId;
    ...
    mSubMap = mNh.subscribe<ccmslam_msgs::Map>(
        MapInTopicName, params::comm::server::miSubMapBufferSize,
        boost::bind(&Communicator::MapCbServer, this, _1));
    ...
    *ss << "MapOut" << SysType << mClientId;
    PubMapTopicName = ss->str();
    mPubMap = mNh.advertise<ccmslam_msgs::Map>(
        PubMapTopicName, params::comm::server::miPubMapBufferSize);
  } else {
    cout << "\033[1;31m!!!!! ERROR !!!!!\033[0m "
            "Communicator::ResetIfRequested(): invalid systems state: "
         << mpCC->mSysState << endl;
  ...
  if (MapInTopicName == "nospec") {
    cout << "Client " << mClientId
         << " \033[1;31m!!!!! ERROR !!!!!\033[0m "
            "Communicator::Communicator(...): bad IN topic name"
         << endl;
...
void Communicator::RunClient() {
  while (true) {
    ...
    for (list<kfptr>::iterator lit = mlpAddedKfs.begin();
         lit != mlpAddedKfs.end(); ++lit) {
      kfptr pKFi = *lit;
...
#ifdef INTERRUPTBA
  if (pMsg->KFUpdates.size() > 0 || pMsg->Keyframes.size() > 0 ||
      pMsg->MPUpdates.size() || pMsg->MapPoints.size() > 0) {
    ...
    if (mpMap->GetMaxKFid() > 10) {
      ...
      const size_t nThresFinished =
          (size_t)(5.0 * params::comm::client::mfPubFreq);
      // idea: if for n sec only empty msgs arrive, this client should be
      // finished. With working connection, in n sec arrive n*Pub_freq_client
      // msgs
      if (mnEmptyMsgs >= nThresFinished) {
        ...
        if (mpMap->isGBAinterrupted() && !mpMap->isRunningGBA()) {
          ...
          mnEmptyMsgs = 0; // reset this value, otherwise a finished module will
                           // request BA in every interation -- not good if
                           // other module is still running
        }
#endif
      }
    }
...
void Communicator::PublishMapClient() {
  ...
  if (mpMap->KeyFramesInMap() <=
      params::tracking::miInitKFs) // starting sending not before tracking is
                                   // initialized stably
    return;
  ...
  // Keyframes
  {
    ...
    while (!mspBufferKfOut.empty() &&
           (ItCount < mKfItBoundPub)) // Do not call checkBuffer()-method -- it
                                      // also need the BufferOutMutex
    {
      ...
      int nFullKFs =
          msgMap.Keyframes.size(); // we need this to determine whether a full
                                   // KF was added or not
      ...
      if (msgMap.Keyframes.size() > nFullKFs) {
        ...
        if (pCurKf->mId.first > mnMaxKfIdSent &&
            pCurKf->mId.second == mClientId)
      ...
      if (ItCount == mKfItBoundPub) {
        cout << "\033[1;34m!!! NOTICE !!!\033[0m " << __func__ << ":"
             << __LINE__ << " reached OUTPUT KF iteration limit [buffsize: "
             << mspBufferKfOut.size() << "]" << endl;
      }
#endif
    }
  }
  ...
  if (!pKFFront &&
      mpCC->mpCH->GetTrackPtr()->mState == Tracking::eTrackingState::OK) {
    return;
  } else
  ...
  // MapPoints
  {
    ...
    while (!mspBufferMpOut.empty() &&
           (ItCount < mMpItBoundPub)) // Do not call checkBuffer()-method -- it
                                      // also need the BufferOutMutex
    {
      ...
      int nFullMPs =
          msgMap.MapPoints.size(); // we need this to determine whether a full
                                   // MP was added or not
      ...
      if (ItCount == mMpItBoundPub) {
        cout << "\033[1;34m!!! NOTICE !!!\033[0m " << __func__ << ":"
             << __LINE__ << " reached OUTPUT MP iteration limit -- size: "
             << mspBufferMpOut.size() << endl;
      }
#endif
    }
  }
  ...
  bool bSend = !msgMap.Keyframes.empty() || !msgMap.KFUpdates.empty() ||
               !msgMap.MapPoints.empty() || msgMap.MPUpdates.empty() ||
               msgMap.ClosestKf_Id != KFRANGE;
...
void Communicator::PublishMapServer() {
    ...
    if (mpNearestKF && mNearestKfId.first != KFRANGE) {
      if (mpNearestKF->mId != mNearestKfId) {
        ...
        if (pKF)
          ...
          for (size_t itclose = 1; itclose < 1 + SERVERCURKFSEARCHITS;
               ++itclose) {
            pKF = mpMap->GetKfPtr(mNearestKfId.first - itclose,
                                  mNearestKfId.second);
            if (pKF) {
              mpNearestKF = pKF;
              break;
  ...
  if (!msgMap.vAckKFs.empty() || !msgMap.vAckMPs.empty() ||
      !msgMap.Keyframes.empty() || !msgMap.MapPoints.empty() ||
      !(msgMap.WeakAckKF == KFRANGE) || !(msgMap.WeakAckMP == MPRANGE)) {
    ++mServerMapCount;
...
void Communicator::ProcessKfInClient() {
  ...
  while (!mlBufKFin.empty() && (ItCount < mKfItBound)) {
    ...
    if (msgpair.first.mClientId != MAPRANGE) {
      ...
      *pMsg = msgpair.first;
      ...
      if (pKF) {
        ...
        pKF.reset(new KeyFrame(pMsg, mpVoc, mpMap, mpDatabase,
                               shared_from_this(), mpCC->mSysState));
      ...
      cout << "\033[1;31m!!!!! FATAL ERROR !!!!!\033[0m " << __func__ << ":"
           << __LINE__ << " Reception of reduced KF not implemented" << endl;
      throw estd::infrastructure_ex();
    }
    ...
    if (ItCount == mKfItBound) {
      cout << "\033[1;34m!!! NOTICE !!!\033[0m " << __func__ << ":" << __LINE__
           << " -- Agent " << mpCC->mClientId
           << ": reached INPUT KF iteration limit"
           << "[" << mlBufKFin.size() << "]" << endl;
    }
#endif
  }
  ...
  while (!mlBufKFin.empty() && (ItCount < mKfItBound)) {
    ...
    if (msgpair.first.mClientId != MAPRANGE) {
      ...
      *pMsg = msgpair.first;
      ...
      kfptr pKF{new KeyFrame(pMsg, mpVoc, mpMap, mpDatabase, shared_from_this(),
                             mpCC->mSysState, mpCC->mpUID->GetId(),
                             mpCC->mg2oS_wcurmap_wclientmap)};
        ...
        if (pKFp) {
          if (pKFp->mId == pKF->mId) {
            std::cout << COUTERROR << "KF " << pKF->mId.first << "|"
                      << pKF->mId.second << " : is its own parent" << std::endl;
          }
        } else {
          if (!(pKF->mId.first == 0))
            std::cout << COUTERROR << "KF " << pKF->mId.first << "|"
                      << pKF->mId.second << " : no parent" << std::endl;
        }
      }
#endif
      ...
      *pMsg = msgpair.second;
      ...
      if (!pKF) {
        delete pMsg;
        continue; // maybe it is deleted, maybe it got lost -- but it is not
                  // there
      }
...
void Communicator::ProcessMpInClient() {
  ...
  while (!mlBufMPin.empty() && (ItCount < mMpItBound)) {
    ...
    if (msgpair.first.mClientId != MAPRANGE) {
      ...
      *pMsg = msgpair.first;
      ...
      if (pMP) {
        ...
        pMP.reset(
            new MapPoint(pMsg, mpMap, shared_from_this(), mpCC->mSysState));
    ...
    if (ItCount == mMpItBound) {
      cout << "\033[1;34m!!! NOTICE !!!\033[0m " << __func__ << ":" << __LINE__
           << " -- Agent " << mpCC->mClientId
           << ": reached INPUT MP iteration limit"
           << "[" << mlBufMPin.size() << "]" << endl;
    }
#endif
  }
  ...
  while (!mlBufMPin.empty() && (ItCount < mMpItBound)) {
    ...
    if (msgpair.first.mClientId != MAPRANGE) {
      ...
      *pMsg = msgpair.first;
      ...
      mpptr pMP{new MapPoint(pMsg, mpMap, shared_from_this(), mpCC->mSysState,
                             mpCC->mpUID->GetId(),
                             mpCC->mg2oS_wcurmap_wclientmap)};
...
void Communicator::DeleteMpFromBuffer(mpptr pMP) {
  ...
  set<mpptr>::iterator sit =
      find(mspBufferMpOut.begin(), mspBufferMpOut.end(), pMP);
  if (sit != mspBufferMpOut.end()) {
...
void Communicator::ResetMapping() { mpMapping->RequestReset(); }

void Communicator::ResetMap() { mpMap->clear(); }
